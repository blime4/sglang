...
top kernels by time. This is the ground-truth data for where the
405ms/token goes.
"""
...
CG = os.environ.get("CG", "0") == "1"
MEM_FRAC = os.environ.get("MEM_FRAC", "0.82")
CTX = os.environ.get("CONTEXT_LEN", "4096")
MRR = os.environ.get("MAX_RUNNING_REQUESTS", "16")
N_PROFILE = int(os.environ.get("N_PROFILE", "1"))  # how many decode steps to profile

os.environ.setdefault("SGLANG_DL_MOE_DLBLAS", "1")  # use dlblas MoE
...
engine = sgl.Engine(
    ...
    disable_cuda_graph=CG == False if False else True,  # always disable for profiling
    attention_backend="fa3",
    chunked_prefill_length=8192,
)
print(f"[profile] engine up. CG disabled for clean timing.", flush=True)

# warmup (with temperature=0 so greedy JIT is paid before timing)
warm_prompt = "Explain the theory of relativity in detail."
for _ in range(2):
    r = engine.generate(warm_prompt, {"max_new_tokens": 16, "temperature": 0.0})
print(f"[profile] warmup done.", flush=True)

# Now profile N decode steps via generate with a fresh short prompt
prof_prompt = "Write a long essay about artificial intelligence."

# Hook: profile a chunk of generation
...
with prof.profile(
    ...
    # generate tokens — each new token is a decode step
...
# Summarize from the key_averages
print("\n" + "=" * 70)
