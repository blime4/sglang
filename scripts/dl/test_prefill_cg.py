#!/usr/bin/env python3
"""Test prefill CG (breakable backend) — does it capture + speed up 2K prefill?
PASS_CG_PREFILL=breakable (default) or 'disabled' (control)."""
import os, time
...
def main():
    ...
    cg_prefill = os.environ.get("CG_PREFILL", "breakable")
    print(f"[prefill-cg] backend={cg_prefill}", flush=True)
    kwargs = dict(model_path=MODEL, tp_size=4, dtype="bfloat16", context_length=4096,
    ...
    if cg_prefill != "disabled":
        kwargs["cuda_graph_backend_prefill"] = cg_prefill
    e = sgl.Engine(**kwargs)
    sp = {"max_new_tokens": 1, "temperature": 0}
    e.generate(input_ids=list(range(100, 30000))[:2048], sampling_params=sp)  # warmup
    ts = []
    for s in [1, 2, 3]:
    ...
    print(f"[prefill-cg] backend={cg_prefill} 2K reps={[round(t*1000) for t in ts]}ms "
          f"median={statistics.median(ts)*1000:.0f}ms ({2048/statistics.median(ts):.0f} tok/s)", flush=True)
    e.shutdown(); print("[prefill-cg] DONE", flush=True)
